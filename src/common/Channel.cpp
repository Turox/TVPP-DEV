...
bool Channel::allsubChannelFull() {
    ...
    for (map<string, SubChannelServerAuxData>::iterator i = channel_Sub_List.begin();
         i != channel_Sub_List.end(); i++) {
...
bool Channel::AddPeerChannel(Peer *peer) {
    ...
    } else //FLASH CROWD
    {
        ...
        if (this->GetPeerListSizeChannel_Sub(this->posInsertSubChannel->second.Get_ServerAuxChannelId_Sub()) <
            (this->maxPeerInSubChannel)) {
            peerList[peer->GetID()] = PeerData(peer);
            ...
            for (map<string, SubChannelServerAuxData>::iterator i = posInsertSubChannel;
                 i != channel_Sub_List.end(); i++) {
                if (i->second.Get_ServerAuxChannelId_Sub() !=
                    posInsertSubChannel->second.Get_ServerAuxChannelId_Sub()) {
...
void Channel::Create_All_ChannelSub() {
    ...
    for (map<string, SubChannelCandidateData>::iterator i = server_Sub_Candidates.begin();
         i != server_Sub_Candidates.end(); i++)
        ...
        if (channel_Sub_List.count(i->first) == 0) {
            ...
            if (size == 0) {
                channelID_New++;
                size = sizeCluster;
                if ((peerServerAuxNew.size() == sizeCluster) &&
                    (channel_Sub_List.size() < (maxSubChannel * sizeCluster))) {
                    for (unsigned int pos = 0; pos < sizeCluster; pos++) {
                        channel_Sub_List[peerServerAuxNew.at(pos)] = SubChannelServerAuxData(channelId, channelID_New,
                                                                                             peerList[peerServerAuxNew.at(
                                                                                                     pos)].GetPeer(),
                                                                                             GenerateAllLogs);
                    }
...
void Channel::analizePeerToBeServerAux(Peer *source) {
    if (this->channelMode == MODE_NORMAL) {
        boost::mutex::scoped_lock channelSubCandidatesLock(*channel_Sub_Candidates_Mutex);
        if ((this->HasPeer(source)) && (server_Sub_Candidates.size() < this->maxServerAuxCandidate) &&
            (source->GetID() != this->GetServer()->GetID())) {
...
void Channel::printChannelProfile() {
    ...
    for (map<string, SubChannelServerAuxData>::iterator i = channel_Sub_List.begin();
        ...
        cout << "[" << i->first << "] scID [" << i->second.Get_ServerAuxChannelId_Sub() << "] TPeerSub ["
             << totalPeerInSubChannel << "]" << endl;
    }
    cout << "###-------" << endl;
}
    ...
    switch (New_channelMode) {
        ...
        case MODE_FLASH_CROWD_MESCLAR: //***********************************************************
        {
            if ((this->channelMode == MODE_FLASH_CROWD) || (this->channelMode == MODE_FLASH_CROWD_MESCLAR)) {
                ...
                cout << "Is not permitted change the state channel " << this->channelMode << " to " << New_channelMode
                     << endl;
        ...
        case MODE_FLASH_CROWD: //*********************************************************************
        {
            if (this->channelMode == MODE_NORMAL) {
                ...
                if (server_Sub_Candidates.size() > maxSubChannel * sizeCluster) {
                    for (unsigned int i = 0; i < (maxSubChannel * sizeCluster); i++) {
                        ...
                        int sorteado;
                        sorteado = rand() %
                                   server_Sub_Candidates.size(); // Retorna um numero aleatorio entre 0 e tamanho do mapa
                        ...
                        subCandidatosTempList.insert(subCandidatosTempList.end(),
                                                     std::pair<string, SubChannelCandidateData>((*j).first,
                                                                                                (*j).second));
                ...
                */
                for (map<string, PeerData>::iterator peerRedePrincipal = this->peerList.begin();
                     peerRedePrincipal != peerList.end(); peerRedePrincipal++)
            ...
            } else if (this->channelMode == MODE_FLASH_CROWD_MESCLAR)
                cout << "Is not permitted change the state channel " << this->channelMode << " to " << New_channelMode
                     << endl;
...
bool Channel::Finishing_Server(string *server) {
    for (map<string, SubChannelServerAuxData>::iterator i = channel_Sub_List.begin();
         i != channel_Sub_List.end(); i++) {
        if ((channel_Sub_List[*(server)].Get_ServerAuxChannelId_Sub() == i->second.Get_ServerAuxChannelId_Sub()) &&
            (*(server) != i->first))
            return false;
    }
    return true;
...
vector<PeerData *> Channel::MakeServerAuxList() {
    ...
    for (map<string, SubChannelCandidateData>::iterator c = server_Sub_Candidates.begin();
         c != server_Sub_Candidates.end(); c++)
...
vector<PeerData *> Channel::SelectPeerList(Strategy *strategy, Peer *srcPeer, unsigned int peerQuantity,
    ...
    if (((this->GetChannelMode() != MODE_FLASH_CROWD) && (this->GetChannelMode() != MODE_FLASH_CROWD_MESCLAR)) ||
        (modeFlasCrowdSemSubChannel)) {
        for (map<string, PeerData>::iterator i = peerList.begin(); i != peerList.end(); i++)
    ...
    } else  //Modo flash crowd com subcanais isolados ou mesclando
    {
        ...
        for (map<string, PeerData>::iterator i = peerList.begin(); i != peerList.end(); i++) {
            if ((srcPeer->GetID() != i->second.GetPeer()->GetID()) && (         // (se ele não é o mesmo escolhido)  E
                    (srcPeerChannelId_Sub == i->second.GetChannelId_Sub()) ||
                    // (   (se o subChannel_ID = subChannel_ID) ou
                    ((srcPeerChannelId_Sub < 0) && (srcPeer->GetSizePeerListOutInformed() > 0) &&
                     //     ((está mesclando) e (ele não é free) e
                     (i->second.GetChannelId_Sub() == (int) this->channelId)) ||
                    //     (escolhido é da rede principal)) ou
                    ((srcPeerChannelId_Sub == (int) this->channelId) &&
                     //     (ele é da rede principal e escolhido está mesclando) ) e
                     (i->second.GetChannelId_Sub() < 0))) && (
                        channel_Sub_List.find(i->first) ==
                        channel_Sub_List.end())                                    // (não fornece server aux para par da rede principal)
                    ) {
...
void Channel::PrintPeerList() {
    cout << "Channel [" << channelId << "] Tip[" << serverNewestChunkID << "] Rate[" << serverEstimatedStreamRate
         << "] Peer List:" << endl;
    for (map<string, PeerData>::iterator i = peerList.begin(); i != peerList.end(); i++)
        cout << "PeerID: " << i->first << " Mode: " << (int) i->second.GetMode() << " TTL: "
             << i->second.GetTTLChannel() << endl;
}
