...
int main(int argc, char *argv[]) {
    ...
    if (arg1 == "--help") {
        ...
        cout
                << "  -peerListSharedSize          define the peer quantity to be shared each time between bootstrap and peer ()(default: "
                << peerListSharedSize << ")" << endl;
        cout << "  -minimalOUTsend              define the minimum OUT to share a peer                     (defautl: "
             << (int) minimumBandwidth << ")" << endl;
        cout << "  -minimalOUTFREEsend          define the minimum OUT_FRER to share a peer to Free Rider  (defautl: "
             << (int) minimumBandwidth_FREE << ")" << endl;
        cout << endl;
        cout << "      ****  FLASH CROWD ***" << endl;

        cout << "  -maxSubChannel               define the sub channel limit                  (default: "
             << maxSubChannel << ")" << endl;
        cout << "  -maxServerAuxCandidate       define the server limit to create sub channel (default: "
             << maxServerAuxCandidate << ")" << endl;
        cout << "  -maxPeerInSubChannel         define the peer number in new sub channel     (default: "
             << maxPeerInSubChannel << ")" << endl;
        cout << "  -sizeCluster                 define the cluster size for each sub channel  (default: " << sizeCluster
             << ")" << endl;
        cout << "  -mergeType                   define the server_aux merge mode              (default: " << mergeType
             << ")" << endl;
        cout
                << "                                **(s-kill 0, s-avoid-p 1, s-permit-p 2, s-avoid-kill 3, s-permit-kill 4, s-noting 5)"
                << endl;
        cout << "  -qt_PeerMergeType            define the peer quantity to be disconnected   (default: "
             << (int) qt_PeerMergeType << ")" << endl;
        cout << "  -timeDescPeerMerge           define the time interval to disconnect peers  (default: "
             << (int) timeDescPeerMerge << ")" << endl;
    ...
    Bootstrap bootstrapInstance(myUDPPort, peerlistSelectorStrategy, peerListSharedSize, maxSubChannel,
                                maxServerAuxCandidate,
                                maxPeerInSubChannel, sizeCluster, mergeType, qt_PeerMergeType, timeDescPeerMerge,
                                minimumBandwidth, minimumBandwidth_FREE);
